...
public class FoodView extends CssLayout implements View {
	...
	public FoodView() {
		...
		this.addComponent(new HorizontalLayout(this.upload, this.uploadBar, this.deleteImage));
		...
		this.upload.addFinishedListener(new Upload.FinishedListener() {
			...
			public void uploadFinished(FinishedEvent event) {
				...
				imageService.processFile(fileService.getFile(event.getFilename()));
			}
		});
		...
		this.imageService.addProcessingListener(new ImageService.ProcessingListener() {
			...
			public void processingSuccess(final ProcessingEvent event) {
				UI.getCurrent().access(new Runnable() {
					public void run() {
						...
						saveChanges.click();						
						image.setSource(new FileResource(event.getFile()));
					}
				});
			}
			
			@Override
			public void processingFailure(final ProcessingEvent event, final Exception cause) {
				UI.getCurrent().access(new Runnable() {
					public void run() {
						uploadBar.setVisible(false);
						cause.printStackTrace();
						fileService.deleteFile(event.getFile().getName());
						Notification.show("The uploaded file is not a valid image file, or it cannot be resized.\n", cause.getMessage(), Notification.Type.ERROR_MESSAGE);						
					}
				});
			}
	...
	private void updateView(boolean editMode) {
		UI.getCurrent().setPollInterval(editMode ? 200 : -1);
