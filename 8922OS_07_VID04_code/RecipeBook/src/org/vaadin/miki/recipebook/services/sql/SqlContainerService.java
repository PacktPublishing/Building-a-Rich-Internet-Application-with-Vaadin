...
public class SqlContainerService implements ContainerService {
	...
	public SQLContainer getRecipesContainer(Map<String, String> filters,
		...
		for(String attribute: new String[]{RECIPE_INGREDIENTS_ATTRIBUTE, RECIPE_TAG_ATTRIBUTE})
			if(filters.containsKey(attribute)) {
				String[] attributeValues = filters.get(attribute).split("\\s*,\\s*");
	...
	@Override
	public SQLContainer getCommentsContainer(Object recipeId) {
		SQLContainer comments = this.getCommentsContainer();
		comments.addContainerFilter(new Compare.Equal("recipe_id", recipeId));
		return comments;
	}
